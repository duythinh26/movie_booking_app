    ...
    return movie ? (
        <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
            <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
                <img src={movie.movie.poster_path} alt="" className="max-md:mx-auto rounded-xl h-104 max-w-70 object-cover"/>

                <div className="relative flex flex-col gap-3">
                    <BlurCircle top='-100px' left='-100px'/>
                    <p className="text-primary">English</p>
                    <h1 className="text-4xl font-semibold max-w-96 text-balance">{movie.movie.title}</h1>
                    <div className="flex items-center gap-2 text-gray-300">
                        <StarIcon className="w-5 h-5 text-primary fill-primary"/>
                        {movie.movie.vote_average.toFixed(1)} Đánh giá từ người dùng 
                    </div>
                    <p className="text-gray-400 mt-2 text-sm leading-tight max-w-xl">{movie.movie.overview}</p>

                    <p className="">{timeFormat(movie.movie.runtime)} • {movie.movie.genres.map((genre) => genre.name).join(", ")} • {movie.movie.release_date.split("-")[0]}</p>
            ...
            <div className="overflow-x-auto no-scrollbar mt-8 pb-4">
                <div className="flex items-center gap-4 w-max px-4">
                    {movie.movie.casts.slice(0, 10).map((cast, index) => (
            ...
            <p className="text-lg font-medium mt-20 mb-8">Các phim cùng thể loại</p>
